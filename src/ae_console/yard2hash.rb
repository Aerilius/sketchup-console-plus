...
#require 'yard'
require 'json'
#require 'awesome_print' # optional

begin
  require 'yard'
rescue LoadError
  begin
    load '/usr/lib/ruby/vendor_ruby/yard.rb'
  rescue LoadError, Errno::ENOENT
    load 'Z:/usr/lib/ruby/vendor_ruby/yard.rb'
  end
end
begin
  ...
  class Console
    ...
    class Yard2Hash
      ...
      def to_hash(node_paths, options={})
        ...
        #}
        YARD::Registry.all(*node_paths).each { |node|
      ...
      # Walk over the yard registry tree and add node data to a hash.
        ...
        if node.type == :method
          if node.has_tag?(:param)
            node_hash[:parameters] = node.tags(:param).map { |tag|
              [tag.name, tag.types, tag.text] # TODO: default value
            }
          end
          if node.has_tag?(:return)
            tag = node.tag(:return)
            node_hash[:return] = [tag.types && tag.types.first, tag.text]
          end
        end
        ...
        def main
          ...
          if output_file
            if File.exists?(File.dirname(output_file))
              Yard2Hash.new(input_directory).to_file(output_file, nodes, options)
            else
              puts 'The directory for the given output file does not exist. Nothing exported.'
              exit 1
            end
          else
            Yard2Hash.new(input_directory).to_stdout(nodes, options)
          end
        end
